...
export async function initStore(dbPath?: string): Promise<Store> {
  ...
  try {
    ...
    throw new Error(`Could not acquire lock on database: ${error instanceof Error ? error.message : String(error)}`);
  }
  ...
  if (existsSync(finalDbPath)) {
    try {
      ...
      logger.warn(`Database file is corrupt. Renaming to ${corruptPath} and creating fresh database.`);
