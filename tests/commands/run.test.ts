...
vi.mock('../../src/utils/progress', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../src/utils/progress')>()
  return {
    ...actual,
...
describe('runPipeline', () => {
  ...
  it('should handle mixed sources: 2 RSS + 1 web in same config', async () => {
    ...
    vi.mocked(fetchRssFeed)
      .mockResolvedValueOnce({
        items: [
          { id: 'rss-1', title: 'RSS 1', link: 'https://a.com/1', content: 'C1', sourceName: 'HN' },
        ],
        errors: [],
      })
      .mockResolvedValueOnce({
