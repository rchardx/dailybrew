...
vi.mock('openai', () => {
  ...
  class APIError extends Error {
    status: number
    constructor(status: number, _body: any, message: string, _headers: any) {
      super(message)
      this.status = status
    }
  }
...
function makeLLMStructuredResponse(data: { title: string; summary: string; importance: number }) {
  return {
    choices: [
      {
        message: {
...
function makeWebPageHtml(articles: Array<{ href: string; title: string }>): string {
...
describe('Integration: Full Brew Pipeline', () => {
  ...
  it('dedup across runs: second run with same feed produces fewer items', async () => {
    ...
    const configPath = writeConfig(tmpDir, {
      llm: { baseUrl: 'http://localhost:11434/v1', apiKey: 'test-key', model: 'test-model' },
      sources: [{ name: 'DedupFeed', url: 'https://dedup-feed.com/rss', type: 'rss' }],
  ...
  it('mixed sources: config with 1 RSS + 1 web source → both appear in output', async () => {
    ...
    const webPage = makeWebPageHtml([{ href: 'https://web-source.com/post-1', title: 'Web Post' }])
  ...
  it('--output flag: verify file is created with correct content', async () => {
    ...
    const configPath = writeConfig(tmpDir, {
      llm: { baseUrl: 'http://localhost:11434/v1', apiKey: 'test-key', model: 'test-model' },
      sources: [{ name: 'OutputFeed', url: 'https://output-feed.com/rss', type: 'rss' }],
  ...
  it('importance sorting in output: 🔴 before 🟢', async () => {
    ...
    // Content ordering
    expect(result.indexOf('Critical Breaking News')).toBeLessThan(result.indexOf('Normal News'))
    expect(result.indexOf('Normal News')).toBeLessThan(result.indexOf('Low Priority News'))
