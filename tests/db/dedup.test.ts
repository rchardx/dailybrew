...
describe('Deduplication Logic', () => {
  ...
  describe('markSeen', () => {
    it('should insert a new seen item', () => {
      ...
      const result = store!.db.exec(`SELECT id, source, title FROM seen_items WHERE id = 'test-id-1'`);
    ...
    it('should not create duplicates for same id', () => {
      ...
      const result = store!.db.exec(`SELECT COUNT(*) as count FROM seen_items WHERE id = 'duplicate-id'`);
    ...
    it('should update last_seen timestamp on duplicate insert', () => {
      ...
      const firstResult = store!.db.exec(`SELECT first_seen, last_seen FROM seen_items WHERE id = 'update-test'`);
      ...
      const secondResult = store!.db.exec(`SELECT first_seen, last_seen FROM seen_items WHERE id = 'update-test'`);
  ...
  describe('setLastRunTime', () => {
    ...
    it('should update existing last run timestamp', () => {
      ...
      const result = store!.db.exec(`SELECT COUNT(*) as count FROM meta WHERE key = 'last_run_time'`);
      expect(result[0].values[0][0]).toBe(1);
    });
